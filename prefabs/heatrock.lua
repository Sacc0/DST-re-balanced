GLOBAL.TUNING.HEAT_ROCK_CARRIED_BONUS_HEAT_FACTOR = 1.0
...
local function TemperatureChange(inst, data)
    ...
    if range ~= inst.currentTempRange then
        ...
        if (inst.lowTemp ~= nil and range >= 3) or
            ...
            inst.lowTemp = nil
            inst.highTemp = nil
            inst.components.fueled:SetPercent(inst.components.fueled:GetPercent() - 1 / GLOBAL.TUNING.HEATROCK_NUMUSES)
        end
    end
end
...
local function fn()
    ...
    inst.scrapbook_fueled_rate = GLOBAL.TUNING.HEATROCK_NUMUSES
    ...
    inst.components.temperature.inherentinsulation = GLOBAL.TUNING.INSULATION_SMALL
    inst.components.temperature.inherentsummerinsulation = GLOBAL.TUNING.INSULATION_SMALL
    inst.components.temperature:IgnoreTags("heatrock")

    inst:AddComponent("heater")
    inst.components.heater.heatfn = HeatFn
    inst.components.heater.carriedheatfn = HeatFn
    inst.components.heater.carriedheatmultiplier = GLOBAL.TUNING.HEAT_ROCK_CARRIED_BONUS_HEAT_FACTOR
    inst.components.heater:SetThermics(false, false)
